...
- **One friendly accent, doing all the work.** A single accent color (a warm, calm teal family) carries emphasis, energy, and "this is the important bit." One accent, not a palette, because **a rainbow is overwhelm** — every additional color is one more thing for a nervous eye to decode. One accent color is a calm, confident voice; six colors is a crowded room. The accent should feel encouraging, not loud. The teal is deliberately warm-leaning rather than clinical, so it still reads as approachable next to the paper — but it is chosen to sit clearly away from the clay/terracotta-orange band, so a recorded walkthrough never risks visually echoing an AI vendor's brand identity. Warmth in this system is carried primarily by the paper canvas and the typography; the accent's job is to feel friendly and encouraging, which a calm teal does without pulling toward any tool's house color.
...
*The how (current, derivable, replaceable):* warm paper around `#fdfcf9`→`#f2eee6` (lifted lighter than the original `#fafaf7`→`#ece8de` so the teal accent separates cleanly while staying unmistakably warm); one warm-teal accent — `--accent` `#0f8a80` for fills, buttons, borders and large headline emphasis, with deeper shades `--accent-deep` `#0c6f67` (links, subheads) and `--accent-ink` `#0a5952` (text-safe on tinted panels and prompt boxes) for text-on-paper; a single reserved green (`#16a34a`) used only for "done / new," never decoratively; warm near-black ink for reading. The tokens are named `--accent*`, not by hue, so the color can move again without renaming. These values serve the feeling; if a warmer or calmer set serves it better, change them — but keep the accent clear of the clay/terracotta-orange band for the vendor-neutrality reason above.
