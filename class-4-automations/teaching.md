...
Do-it-yourself by design — DON'T walk them through it. They add ONE source that's specific to their work — the one that makes the brief genuinely theirs (Todoist, Slack, their Class 2 dashboard, or something only they'd track). Your role if consulted: reinforce that it's the SAME pattern they did three times today — tell Claude the source, have it build a new SEPARATE reader, test it alone, add it to the brief; plan mode, a copy-paste key in .env, done. They know plan mode, reference docs, .env keys, and the reader pattern — this is them proving they can do it independently. Resist doing it for them; coach the pattern.
