...
| `../content-guide/GEO_CONTENT_GUIDE.md` | project **root** | The writing standard for every bot page. **Copy it into the project** — the student keeps writing bot pages long after class, and this is what tells Claude how. |
...
4. **`GEO_CONTENT_GUIDE.md` → project root, and wire it in (see below).** This is
...
## Make the GEO standard permanent (don't skip this)
...
- Anything in `llm/` is the AI/bot version of the site. When writing or updating
  ANY file in `llm/`, follow `GEO_CONTENT_GUIDE.md` in the project root — it is
  the writing standard for those pages. Read it first; don't write from memory.
- Never apply the GEO guide to the human site. It governs `llm/` only.
...
Claude writes marketing copy into `llm/` instead of answer-first GEO content.
