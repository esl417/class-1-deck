...
- `dashboard/` was designed that way. A dashboard build is conversational, so
  ...
  no shared reconvergence point. **Nothing is built live**: the real build takes
  ~16 minutes against ~14 minutes of available teaching, so slide 3 shows the
  prompt (disclosing that it was run beforehand) and slide 8 opens the
  `dashboard.html` that prompt produced. Timed and decided 2026-09-08.
...
opened in the browser, and nothing else. It is generated by
`dashboard/build_dashboard.py` from `dashboard/sample-data/`, and it is a real
instrument rather than a static render: four assumption inputs (minutes per
return, CS headcount, hours each, wage) recompute every figure live, and the
narrative verdicts are derived from the data rather than hardcoded — feeding it
an export where a different channel is the problem flips the headline and
rewrites the prose. The honest limit (a local file does not
...
### The dashboard lesson's numbers are derived, never typed

Every figure quoted on a `dashboard/` slide (93%, 112 returns, 13 vs 2.5
minutes, growth room, …) is computed from `dashboard/sample-data/orders.csv`.
`sample-data/verify_deck_claims.py` recomputes all of them and fails loudly on
drift — run it after any change to `generate.py` or to a figure on a slide.

This exists because the deck once carried a set of numbers the data did not
produce: they were `generate.py`'s design target, copied into the slides as if
they were fact, and a live build would have contradicted the deck on stage.
**The CSV is the source of truth — not the docstring, not the slides.** If the
verifier fails, fix the deck.

`generate.py` simulates `HISTORY_DAYS = 180` but writes only the last
`EXPORT_DAYS = 45`, so the export is a recent window of an established business.
Shortening the simulation instead of the export compresses the growth curve and
shrinks the business itself.
